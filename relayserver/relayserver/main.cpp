...
//    }
    ...
    talk_base::scoped_ptr<talk_base::AsyncUDPSocket> int_socket(talk_base::AsyncUDPSocket::Create(pthMain->socketserver(), int_addr));
    if (!int_socket) {
    ...
    talk_base::scoped_ptr<talk_base::AsyncUDPSocket> ext_socket(talk_base::AsyncUDPSocket::Create(pthMain->socketserver(), ext_addr));
    if (!ext_socket) {
        std::cerr << "Failed to create a UDP socket bound at"
